...
class Detection:
    ...
    def loadModel(self, model):
        """
        Compiles model with metrics and loss functions
    ...
    def predict(self, im, paint):
        """
        Predicts input data
        :return: pandas dataframe with output Tensor of Persons
        """
    ...
    def get_feets(self, im, results_pd, paint):
        """
        Given a bounding box return its feets
        :return: position of bounding box feets
        """
        feets = []
    ...
    def detect(self, im, paint=False):
        im, results_pd = self.predict(im, False)
